...
const checked = computed({
  get: () => {
    ...
    _checked.value = props.modelValue === props.name
...
watch(() => _checked.value, val => {
  checked.value = val ? props.name : ''
  _checked.value = props.modelValue === props.name
}, { immediate: true })
...
<template>
  <div :class="{ checked: _checked }" @click="toggle" class="LineSelect">
    <div class="LineSelect-Head">
      <span style="position: absolute;opacity: 0">{{ checked }} | {{ _checked }}</span>
      <label>
        <input v-model="_checked" value="on" name="dummy" type="checkbox" class="bubble">
      </label>
    </div>
    <slot />
  </div>
